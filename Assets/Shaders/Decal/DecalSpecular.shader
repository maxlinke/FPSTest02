Shader "Custom/Decal/DecalSpecular"{
    ...
    SubShader{
        ...
        Pass{
			...
			fixed4 frag(vertex_output i) : SV_Target{
				...
				fixed spec = pow(saturate(dot(halfVector, i.worldNormal)), _SpecSmoothness * 100);
				...
				c.rgb += spec * atten * lightColor * _SpecColor;
				c.a = tex.a;
        ...
        Pass{
			...
			fixed4 frag(v2f i) : SV_Target{
				...
				fixed spec = pow(saturate(dot(reflectionDir, viewDir)), _SpecSmoothness*100);
				fixed diff = saturate(dot(i.worldNormal, i.lightDir));

				fixed4 c;
				c.rgb = tex.rgb * diff * atten * lightColor;
				c.rgb += spec * atten * lightColor * _SpecColor.rgb;
				c.a = tex.a;
